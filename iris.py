...
# Dataset: iris.csv (sepal_length, sepal_width, petal_length, petal_width, species) [web:41][web:47]

import pandas as pd
import matplotlib.pyplot as plt
import seaborn as sns

df = pd.read_csv("C:/Users/Sunil Daggubati/Downloads/iris.csv")  # Ensure iris.csv is in the same folder [web:37][web:41]
print(df)
...
print(f"Average petal_width: {avg_petal_width:.2f}")  # Features are in cm [web:37]
...
plt.tight_layout()
plt.show()  # First image (bar chart) [web:57][web:59]
...
plt.legend()
plt.tight_layout()
plt.show()  # Second image (scatter 1) [web:55][web:58]
...
plt.legend()
plt.tight_layout()
plt.show()  # Third image (scatter 2) [web:55][web:58]

plt.figure(figsize=(8, 6))
numeric_df = df.drop(columns=["species"])
corr_matrix = numeric_df.corr()  # correlation matrix for 4 numeric features [web:46][web:49]
...
print("2. Petal length and petal width are highly positively correlated, forming a near-linear relationship.")  # [web:37][web:46]
print("3. Setosa shows smaller petal measurements and relatively larger sepal width than the other species.")  # [web:37][web:58]
